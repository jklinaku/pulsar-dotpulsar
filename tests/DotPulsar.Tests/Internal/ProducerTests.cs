...
public sealed class ProducerTests : IDisposable
{
    ...
    [Fact]
    public async Task Send_WhenProducingToTopicWithSchemaAndNoSchemaValidation_ShouldAllowAnyBytes()
    {
        var topicName = await _fixture.CreateTopic(_cts.Token);
    ...
    [Fact]
    public async Task Send_WhenProducingToTopicWithSchemaAndHasSchemaValidation_ShouldThrowException()
    {
        var topicName = await _fixture.CreateTopic(_cts.Token, Guid.NewGuid().ToString());
        var pulsarSchema = Schema.AvroISpecificRecord<AvroSampleModel>();
        await _fixture.AddSchemaToExistingTopic(topicName, pulsarSchema.SchemaInfo, _cts.Token);
        await _fixture.EnableSchemaValidationInNamespace(TestUtil.extractTenantAndNameSpaceFromTopic(topicName), _cts.Token);
        var client = CreateClient();
        await using var producer = CreateProducer(client, topicName, Schema.ByteSequence);
        var exception = await Record.ExceptionAsync(producer.Send(Encoding.UTF8.GetBytes("test"), _cts.Token).AsTask);
